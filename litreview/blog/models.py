...
class InscriptionForm(forms.Form):
    ...
    password2 = forms.CharField(required=True,
                                widget=forms.PasswordInput(attrs={'placeholder': "Comfirmer mot de passe"}))
...
class CritiqueTicketForm(forms.Form):
    title = forms.CharField(max_length=128, required=True, label='Titre')
    description = forms.CharField(max_length=8192, label='Commentaire', required=True,
                                  widget=forms.Textarea(attrs={'cols': 50, 'rows': 5}))
    image = models.ImageField(blank=True)
    headline = forms.CharField(max_length=128, label='Titre', required=True,
                               widget=forms.TextInput())
    body = forms.CharField(max_length=8192, label='Commentaire', required=False,
                           widget=forms.Textarea(attrs={'cols': 50, 'rows': 5}))
